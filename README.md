...
| **VR Color Gradients 3D** | A C++ effect plugin: the stock VR gradient, with a Z axis on every point. **Built for Windows and macOS** — [own page](plugins/vr_color_gradients_3d/README.md) |
...
**Plug-ins** — [`plugins/vr_color_gradients_3d/`](plugins/vr_color_gradients_3d/) holds both builds: `ChromaVRGradient3D.aex` for Windows and `ChromaVRGradient3D.plugin` (universal) for macOS. Copy the right one into After Effects' `Plug-ins/Effects/` and restart; it appears under **Effect → Immersive Video**.
...
`VR Color Gradients 3D` is a compiled effect plugin rather than a script, built against the **After Effects SDK 25.6**. Both builds ship in the repo — nobody should need Visual Studio or Xcode to use an effect — with the source beside them in `src/`: a **Windows x64 `.aex`**, and a **universal (arm64 + x86_64) macOS `.plugin`**, ad-hoc signed. Neither has been run in After Effects yet.
